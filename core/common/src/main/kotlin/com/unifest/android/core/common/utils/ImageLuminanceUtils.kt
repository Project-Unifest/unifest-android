...
object ImageLuminanceUtils {
    ...
    private fun calculateBitmapLuminance(bitmap: Bitmap): LuminanceResult {
        ...
        // 성능을 위해 샘플링(모든 픽셀을 다 검사하지 않고 일정 간격으로만 픽셀을 샘플링해서 계산 속도를 높임)
        val step = maxOf(1, width / 50)

        for (y in 0 until sampleHeight step maxOf(1, sampleHeight / 20)) {
        ...
        val averageLuminance = (totalLuminance / (sampleHeight / maxOf(1, sampleHeight / 20) * width / step)).toFloat()
